...
mp-grill-with-docs（必要時）
    → mp-to-issues（GitHub issue 出口，宣告阻擋關係）
...
輕量軌的工作項目以 GitHub issues（輔出口）追蹤；OpenSpec `tasks.md` 仍是正式變更的
canonical 出口，輕量軌不改變這件事。`mp-triage` 分流時會逐項檢查準入條件。
...
| `mp-to-issues` | 將需求拆成垂直切片並宣告阻擋關係，輸出 OpenSpec tasks、issue draft 或本地 Markdown |
