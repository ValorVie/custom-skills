...
本指南說明如何使用 MP 工作入口層、OpenSpec 和相關工具進行功能開發。
...
| 探索期規劃 | `/mp-wayfinder` | 模糊大工作規劃成地圖與決策點，每 session 解一個 |
| 需求對齊 | `/mp-grill-with-docs` | 追問模糊需求、固定專案語言、整理未決問題 |
| 設計驗證 | `/mp-prototype` | 丟棄式原型回答狀態模型或 UI 設計問題 |
| 需求摘要 | `/mp-to-prd` | 將既有對話整理成 PRD 或 OpenSpec brief |
| 工作切片 | `/mp-to-issues` | 拆成垂直切片，宣告阻擋關係，標示 `AFK` / `HITL` |
...
| 軌道 | 適用 | 路徑 |
|------|------|------|
| **探索軌** | 大而模糊、跨多 session、連 proposal 都還寫不出來 | `mp-wayfinder` 地圖 → 決策結晶後轉重量軌 |
| **重量軌** | 正式變更：功能、重構、規格修改 | OpenSpec 完整流程（見下） |
| **輕量軌** | 小變更：不值得開 change，但需要對齊與拆票 | 見「輕量軌」一節 |

三條軌道都使用 Beads 管理執行狀態、認領、依賴與 ready。OpenSpec 保留正式變更工件；GitHub Issues 保留外部入口。

本專案情境範例：

- 「重新設計 skills 分發架構」→ 範圍未明、多個未決策 → 探索軌。
- 「新增一個 CLI 子命令」→ 功能變更、影響規格 → 重量軌。
- 「修正某個 skill 的措辭與觸發語彙」→ 單一文件群、一個 session 完成 → 輕量軌。

### MP + OpenSpec 完整流程（重量軌）

```
想法
    → mp-wayfinder（模糊大工作先開地圖，選用）
    → mp-grill-with-docs（需求澄清與專案語言）
    → mp-prototype（設計問題先做丟棄式原型，選用）
    → mp-to-prd（PRD 或 OpenSpec brief，選用）
    → mp-to-issues（垂直切片與 AFK/HITL）
    → mp-triage（確認哪些可交給 agent，選用）
...
若需求已經很清楚，可以直接從 OpenSpec 開始。若任務已經有完整 specs 和 tasks，不需要回頭使用 MP。
...
```
mp-grill-with-docs（必要時）
    → mp-to-issues（Beads 出口，宣告阻擋關係）
    → 直接實作（superpowers:test-driven-development）
    → code review
```

**準入條件（三項同時成立才可走輕量軌，任一不成立改走重量軌）**：
...
Beads 是輕量軌與重量軌的 canonical 執行出口。OpenSpec `tasks.md` 保留正式變更的
工件進度，GitHub Issues 保留外部需求入口。`mp-triage` 分流時會逐項檢查準入條件。

### 完整 TDD 流程

```
想法 → mp-grill-with-docs（必要時）→ /opsx:explore → /opsx:new → /opsx:continue (×N)
...
- [MP 使用指南](MP-USAGE-GUIDE.md)
- [MP 工作入口層導入指南](mp-workflow-entry-layer.md)
...
在使用任何工具之前，先與 AI 對話釐清你的想法。若想法還很模糊，優先使用 MP 工作入口層，而不是直接建立 OpenSpec change。
...
AI：了解。這個功能需要：
    ...
    我建議先用 mp-grill-with-docs 壓清楚需求語言，再決定是否開 OpenSpec。
```
...
**何時進入下一階段**：
當你能用 1-2 句話清楚描述「要解決什麼問題」和「大致怎麼解決」時，就可以進入 MP 工作入口或調研階段。若還有術語、範圍或驗證條件不清楚，先使用 Phase 0.5。

---

### Phase 0.5: MP 工作入口 — 澄清、摘要、切片、分流

MP 工作入口層用於 OpenSpec 之前。它把模糊需求整理成可被 OpenSpec 或 Superpowers 接手的工作。

**常用技能**：

| 技能 | 使用時機 | 產出 |
|---|---|---|
| `mp-wayfinder` | 工作大而模糊，proposal 還寫不出來 | tracker 上的地圖與決策點子項目 |
| `mp-grill-with-docs` | 需求、術語或範圍不清楚 | 已確認語言、決策、未決問題 |
| `mp-prototype` | 狀態模型或 UI 設計問題需要具體驗證 | 丟棄式原型與驗證後的決策 |
| `mp-to-prd` | 已有討論但缺需求摘要 | PRD 或 OpenSpec brief |
| `mp-to-issues` | 需求太大，需要拆工 | 垂直切片、阻擋關係、`AFK` / `HITL` |
| `mp-triage` | 不確定能否交給 agent | `ready-for-agent` / `ready-for-human` 等狀態 |
| `mp-improve-codebase-architecture` | 發現架構摩擦 | 架構改善候選 |
...
AI：我建議先用 mp-grill-with-docs。
第一個需要釐清的是：你要改善的是上游拉取、本地改寫追蹤、分發驗證，還是使用者安裝體驗？
我的建議是先聚焦「本地改寫追蹤」，因為它會影響後續能否安全更新。
```

**對話範例：切成 OpenSpec tasks 與 Beads 工作**

```
你：這個 change 的 tasks 太粗，幫我切成 agent 可以做的任務。

AI：我會用 mp-to-issues，輸出 OpenSpec tasks 格式；需要獨立認領或宣告依賴的切片，
同時建立 Beads 工作。每個切片會標示 AFK 或 HITL，並包含驗證條件。
```

**對話範例：判斷能否交給 agent**

```
你：這個 issue 可以丟給 agent 嗎？

AI：我會用 mp-triage。
若範圍、驗證條件和依賴都清楚，標為 ready-for-agent。
若還需要產品判斷或外部權限，標為 ready-for-human。
```

**何時跳過 MP**：

- 已有完整 proposal、specs、tasks，直接進 `/opsx:apply`。
- 正在做 TDD，直接使用 `superpowers:test-driven-development`。
- 正在除錯，直接使用 `superpowers:systematic-debugging`。
- 正在驗證或歸檔，直接使用 `/opsx:verify` 或 `/opsx:archive`。

更多情境示範請看 [MP 使用指南](MP-USAGE-GUIDE.md)。
...
| 命令 | 說明 |
|------|------|
| `mp-wayfinder` | 模糊大工作規劃成 tracker 地圖，每 session 解一個決策點 |
| `mp-grill-with-docs` | 追問需求、校準專案語言、必要時沉澱 `CONTEXT.md` / ADR |
| `mp-prototype` | 丟棄式原型回答設計問題，產物進 `experiment/` 分支 |
| `mp-to-prd` | 將既有對話整理成 PRD 或 OpenSpec brief |
| `mp-to-issues` | 將需求拆成垂直切片並宣告阻擋關係，輸出 Beads、OpenSpec tasks、issue draft 或本地 Markdown |
| `mp-triage` | 判斷工作項目是否 `ready-for-agent`、`ready-for-human`、`needs-info` 等 |
| `mp-improve-codebase-architecture` | 產出架構改善候選，不直接修改實作 |
