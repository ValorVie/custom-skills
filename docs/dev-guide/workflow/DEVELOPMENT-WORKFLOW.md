...
| `mp-to-issues` | 需求太大，需要拆工 | 垂直切片、`AFK` / `HITL` |
...
| 命令 | 說明 |
|------|------|
| `mp-setup-matt-pocock-skills` | 建立 `docs/agents/` 與 Claude / Codex 入口規則 |
| `mp-grill-with-docs` | 追問需求、校準專案語言、必要時沉澱 `CONTEXT.md` / ADR |
| `mp-to-prd` | 將既有對話整理成 PRD 或 OpenSpec brief |
| `mp-to-issues` | 將需求拆成垂直切片，輸出 OpenSpec tasks、issue draft 或本地 Markdown |
