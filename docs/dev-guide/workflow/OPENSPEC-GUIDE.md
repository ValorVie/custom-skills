...
本指南供已選擇 OpenSpec 的使用者建立、實作、驗證與歸檔變更。
若還在比較方法，先看[開發流程選擇指南](DEVELOPMENT-WORKFLOW.md)。

OpenSpec 以 change（一次變更）保存提案、設計、需求規格與任務清單；正式規格保留
需求與場景，專案任務系統則管理認領與執行狀態。兩者的分工依專案設定，不必另建
第二套任務清單。

## 開始前與閱讀方式

先確認專案已初始化 OpenSpec，並已在目前 AI 工具載入所需命令。本文保留原有的
逐步教學，以 `/opsx:new`、`/opsx:continue`、`/opsx:verify` 等展開流程為例。
上游目前也提供 `/opsx:propose` 一次準備規劃文件；部分展開命令需要選用 expanded
profile（提供較完整命令的設定組合）。這些是不同入口，不必兩套都跑。

命令名稱也會依 AI 工具不同而變化。來源與最新差異見
[OpenSpec 官方說明](https://github.com/Fission-AI/OpenSpec#quick-start)。
本次整理沒有執行初始化、更新設定或完整開發流程。

本指南後半的 `/custom-skills-*` 是另外安裝的測試與報告工具，不是 OpenSpec
內建命令。使用前確認可見；也可用專案原有測試命令取得相同驗收證據。

| 現在的狀態 | 從哪裡開始 |
| --- | --- |
| 尚未釐清問題 | Phase 0 對話探索，或使用自己選擇的需求技能 |
| 想先了解現有程式 | Phase 1 調研目標 |
| 已有明確需求，要保留正式變更 | Phase 2 建立提案 |
| 已有完整規格與任務 | Phase 4 實作，接續原 change |
| 實作完成 | Phase 5 測試，再比對規格並驗證 |
| 驗證完成，要結束變更 | Phase 7 歸檔 |
...
釐清問題與現況
    → proposal 提案
    → specs 需求規格、design 技術設計、tasks 任務
    → 實作與測試
    → 處理規格與實作差異
    → 驗證並歸檔
```

已有足夠成果的階段可以直接接續，不重建相同文件。TDD（先寫失敗測試再實作）
適合可驗證的行為；文件做內容與連結檢查，設定做格式或整合檢查，無法自動驗證的
部分保留手動證據。不要因跳過測試生成，就省略必要驗證。

## 逐步操作

### Phase 0: 對話探索：釐清想法

先確認問題、範圍與可驗證的結果。能從程式或文件查到的現況，讓 AI 先查；
需要你決定的方向，再透過對話釐清。若已有明確需求，可直接接續調研或提案。
...
2. 顯示下一份文件的建立指引；再使用 `/opsx:continue` 建立提案
...
**變更文件**（`spec-driven` 規格設定）：
依工具回報的依賴關係建立；下列為主要文件，不是所有版本都固定相同順序。
...
1. 執行 Phase 4 已建立的相關測試，檢查本次行為
2. 執行適用的回歸、建置或整合檢查
3. 專案有覆蓋率門檻時，檢查是否達標
4. 記錄通過、失敗、跳過與尚未驗證的項目

若發現缺少測試，先判斷是否有未驗證或錯誤行為；需要修正時回到 Phase 4 的
測試與實作迴圈。實作已正確的情況，不必為了這一階段再刻意製造一次失敗。
...
在實作和測試過程中，經常會發現原始設計需要調整。例如：邊界條件的處理方式改變、API 介面微調、新增或移除了某些場景。若最終實作與原始 specs/design 有出入，先判斷是實作錯誤，還是經確認的需求或設計調整。實作錯誤應修正程式；確認要改變的需求與設計，才回寫文件並重驗。不要只為了讓驗證通過，就把規格改成目前程式的行為。
...
1. 比對實作結果與 specs 中的場景描述，先確認差異的原因與處理方向
2. 對已確認的需求或設計變更，編輯 `openspec/changes/<name>/specs/` 下的 spec 檔案
...
**驗證通過的條件**：
- [ ] 任務的實際完成狀態已記錄，取消或延期項目已有說明
- [ ] 本次適用的測試、建置、格式或手動驗證已完成，結果符合驗收要求
- [ ] 專案有覆蓋率門檻時已達標；不適用時記錄理由
- [ ] 實作符合已確認的規格，差異已處理並重新驗證
- [ ] 跳過與尚未驗證項目已分開記錄，沒有當成通過
...
**這個命令會**：
1. 檢查變更文件、任務與主規格的同步狀態
2. 依所用命令及使用者選擇同步主規格，或明確記錄未同步
3. 將 change 移到歸檔位置；一般 repo 結構為 `openspec/changes/archive/YYYY-MM-DD-<change-name>/`

**歸檔後**：
- Change 完成，不再出現在 `openspec list` 中
- 確認 Specs 是否已同步到主規格；歸檔完成本身不代表同步完成
- 可在 `openspec/changes/archive/` 查看歷史記錄；自訂規劃位置以工具回報為準
...
A: 可以接續已完成的階段。已有確認的需求與規格，就不用重做訪談或提案；
已有任務就從實作開始。必要的驗證仍依本次變更與專案要求完成。

**Q: 實作過程中發現設計有問題怎麼辦？**

A: OpenSpec 支援流動式工作流程：
1. 暫停實作
2. 說明證據與建議；涉及需求、範圍或授權的改變先討論，再修改 design.md 或 specs
3. 必要時更新 tasks.md
4. 繼續實作

**Q: 測試後發現實作與 specs 有偏差怎麼辦？**

A: 先用 Phase 5.6 判斷差異原因。若是實作錯誤就修正程式；若需求或設計確定改變，
再修改相應文件並重驗。不要為了消除差異而自動降低驗收條件。
...
歸檔後，依專案規則處理提交、PR 與發布；操作方式見
[Git 工作流程指南](../git/GIT-WORKFLOW.md)。歸檔不代表已合併、推送或部署。

下一項工作需要重新選擇方法時，回到[開發流程選擇指南](DEVELOPMENT-WORKFLOW.md)。
想在現有 change 補上來源查證、監控或發布準備，可參考
[agent-skills 指南](agent-skills-guide.md)，沿用原規格與任務。
