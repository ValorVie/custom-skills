...
| `~/.config/bd/config.yaml` | 目前使用者的 `bd` 工具偏好 | 否；不保存專案工作內容 |
...
`bd remember` 預設是**專案層級**，不是 Claude、Codex、Beads 帳號或整台電腦共用的記憶。
它會寫入目前 `bd where` 解析到的專案 Dolt 資料庫；`bd prime` 與 `bd memories` 預設也只讀取該資料庫。

Beads 中的「全域」可能指不同事情，需分開理解：

| 項目 | 儲存位置或選擇方式 | 預設生效範圍 | 如何帶到其他位置 |
| --- | --- | --- | --- |
| `bd` CLI | Homebrew、npm 或系統 PATH | 使用者或系統；同一執行檔可操作多個專案 | 每台電腦分別安裝 |
| 使用者工具設定 | `~/.config/bd/config.yaml`；舊版位置為 `~/.beads/config.yaml` | 目前作業系統使用者的多個專案 | 不會自動同步到其他電腦 |
| 環境變數 | `BD_*`、`BEADS_*`、`BEADS_ACTOR` 等 | 目前程序、終端或使用者環境，依設定位置而定 | 在其他環境另行設定 |
| 專案工具設定 | `.beads/config.yaml` | 目前專案 | 由一般 Git 通道共享已追蹤的設定檔 |
| 專案資料庫設定 | `bd config set ...` | 目前專案的 Dolt 資料庫 | `bd dolt push`／`bd dolt pull` |
| 工作資料 | 工作項目、依賴、留言、標籤、事件與一般 `bd remember` | 目前專案 | `bd dolt push`／`bd dolt pull` |
| Claude／Codex 專案整合 | `AGENTS.md`、`CLAUDE.md`、專案技能與 hooks 設定 | 目前專案或 clone | 已追蹤檔案由 Git 共享；本機 hook 可能需重新執行 `bd setup` |
| 明確指定的全域資料庫 | `bd <command> --global` 選擇 `beads_global` | 連到同一個共用 Dolt 伺服器的多個專案 | 必須使用同一個伺服器，且每次明確加上 `--global` |
| 本機執行狀態 | 尚未 push 的 Embedded 資料庫變更、鎖定檔、日誌、憑證與伺服器程序 | 目前電腦或 clone | 不會自動共享；資料變更需經 Dolt 同步 |

其中 `--global` 不是雲端帳號記憶，也不代表所有電腦自動可見。它只是把該次指令的資料庫目標，
由目前專案改成 `shared-server` 模式中名為 `beads_global` 的共用資料庫。一般的 `bd prime` 不會同時合併
專案記憶與全域記憶：

```bash
bd prime             # 讀取目前專案資料庫
bd prime --global    # 改讀 beads_global
```

一般專案優先使用預設的專案資料庫。只有確定多個專案都連到同一個共用 Dolt 伺服器，且確實需要
跨專案共享資料時，才使用 `--global`。

```mermaid
flowchart LR
    usercfg["~/.config/bd/config.yaml<br/>使用者工具偏好"] --> cli["bd CLI"]
    projectcfg[".beads/config.yaml<br/>專案工具設定"] --> cli
    env["BD_* / BEADS_*<br/>目前執行環境"] --> cli

    cli -->|"預設"| projectdb[("目前專案 Dolt DB<br/>issues / dependencies / memories")]
    cli -->|"明確加上 --global"| globaldb[("共用伺服器：beads_global")]

    projectdb -->|"bd dolt push"| remote["refs/dolt/data"]
    remote -->|"bd dolt pull"| clone[("同一專案的其他 clone")]
```
...
```bash
cd project-a
bd remember "部署前必須先執行整合測試" --key deploy-test

cd ../project-b
bd memories deploy-test    # 不會讀到 project-a 的記憶
```

剛寫入時，它只存在 `project-a` 目前 clone 的 Dolt 資料庫。要讓同一專案的另一個 clone 或另一台電腦
取得它，需走 Dolt 同步通道：
...
# project-a 的另一個 clone
bd dolt pull
bd memories deploy-test
```

若已設定 `shared-server` 模式，且真的需要跨專案共用，可明確改用 `beads_global`：

```bash
bd remember --global "所有專案發布前都要檢查變更日誌" --key release-changelog
bd memories --global release-changelog
bd prime --global
```

未加 `--global` 的 `bd memories` 或 `bd prime` 不會自動讀取上述全域記憶；反過來，使用 `--global`
時也不是在查詢目前專案資料庫。
...
- 同一專案需要跨 clone 共享：同步專案 Dolt remote，不要改成 `--global`。
- 只有刻意跨專案共享，且共用同一個 Dolt 伺服器：才使用 `--global`。
...
- [設定系統與生效範圍](https://github.com/gastownhall/beads/blob/main/docs/reference/configuration.md)
- [`bd remember` 持久記憶參考](https://github.com/gastownhall/beads/blob/main/docs/cli-reference/remember.md)
