...
| [sync-projects.example.txt](examples/sync-projects.example.txt) | 多專案同步設定檔範例 |
| [wsl-sync-daemon.sh](examples/wsl-sync-daemon.sh) | inotifywait + rsync 單專案同步腳本 |
| [wsl-sync-manager.sh](examples/wsl-sync-manager.sh) | 多專案統一管理腳本 |
| [wsl-sync.service](examples/wsl-sync.service) | systemd 服務範例（開機自啟，統一管理版） |
