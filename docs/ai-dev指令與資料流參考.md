...
| `~/.config/ai-dev/manifests/npx-first-party.yaml` | 第一方 npx skills 的 per-file base、effective hash、decision 與 overlay metadata；不代表 clone ownership | `install`, `update`, `install-npx-skills` |
| `~/.config/ai-dev/overlays/npx-first-party/` | 第一方 skill 的持久本機檔案與 deletion tombstone（刪除標記） | `install`, `update`, `install-npx-skills` |
| `~/.config/ai-dev/backups/npx-first-party/` | per-skill transaction backup；use-upstream 或失敗 recovery 時保留 | `install`, `update`, `install-npx-skills` |
| `~/.config/ai-dev/transactions/npx-first-party/` | 未完成 transaction journal；下一次執行先 recovery，不直接覆寫 | `install`, `update`, `install-npx-skills` |
...
- `--section` / `-s`：只顯示指定區塊；可用值：`tools`、`repos`、`sync`；省略時顯示全部
  - `tools`：顯示核心工具與全域 NPM 套件
  - `repos`：顯示設定儲存庫狀態
  - `sync`：顯示上游同步狀態
...
- `--list` / `-l`：列出目前的開關狀態（不需要其他參數）
- `--dry-run`：只預覽變更，不移動或還原實體資源
...
- `--only` / `-o`：只更新特定工具；可用值：`openspec`、`uds`
