...
| **Antigravity CLI (agy)** | Google 終端 AI 代理（原 Gemini CLI） | 單一執行檔、多代理工作流、終端執行 |
...
| Google 帳號 | Antigravity CLI（agy，備選工具） | 選用 | 執行 `agy` 以 Google 帳號登入；CI 改用 `ANTIGRAVITY_TOKEN` |
...
> 其他 AI 開發工具（OpenCode、Codex 等）會在 `ai-dev install` 過程中一併處理；Antigravity CLI（agy）因改以官方執行檔發行，請依[附錄 A](#附錄-a備選-ai-開發工具)自行安裝。各工具的詳細說明請參閱[附錄 A](#附錄-a備選-ai-開發工具)。
...
> **重要變更**：Google 已於 2026-06-18 關閉舊版 **Gemini CLI**（`gemini` 指令、`@google/gemini-cli` 套件），改由 **Antigravity CLI**（`agy` 指令）接手。agy 以單一執行檔發行，**不再透過 npm 或 Homebrew 安裝**；認證也由 `GEMINI_API_KEY` 改為 `ANTIGRAVITY_TOKEN` 與 Google 帳號登入。
...
安裝程式會將名為 `agy` 的執行檔放到 `~/.local/bin/`（Unix）或 `%LOCALAPPDATA%\Antigravity\`（Windows）。

#### 使用方式

```shell
# 互動式啟動（首次啟動會引導以 Google 帳號登入）
agy
...
# 從舊版 Gemini CLI 匯入既有擴充
agy plugin import gemini
```

CI 環境改用環境變數 `ANTIGRAVITY_TOKEN`（取代舊版的 `GEMINI_API_KEY`），並可加上 `--headless --approve all` 以非互動方式執行。
...
| 日期 | 版本 | 變更內容 |
|------|------|----------|
| 2026-06-25 | 2.2.0 | 舊版 Gemini CLI 已於 2026-06-18 退役，附錄 A 安裝/使用方式改為 Antigravity CLI（`agy`，curl 執行檔安裝、`ANTIGRAVITY_TOKEN` 認證）；同步更新工具總覽表與帳號表 |
