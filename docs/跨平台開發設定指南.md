...
Ghostty 的設定檔位於 `~/.config/ghostty/config`（macOS / Linux）。

```ini
# ===========================
# 字型設定 (Font Configuration)
# ===========================

# 設定主字型 (名稱必須跟系統字型庫顯示的一模一樣)
...
# 如果覺得字體在 Retina 螢幕上太細，可以開啟這個 (類似 macOS 的抗鋸齒加粗)
# 建議開啟，閱讀體驗會接近 VS Code
font-thicken = true

# 關閉連字功能
# font-feature = -calt

# 調整行高 (預設是 1，想寬鬆一點可以設 1.1 或 1.2)
# 這能解決你之前覺得字體「變矮」的視覺感
# adjust-cell-height = 10%

# 調整字元寬度 (通常不需要動，除非字擠在一起)
# adjust-cell-width = 0
```
...
建議使用支援 Nerd Font 圖示與中文的等寬字型，確保終端機與編輯器的顯示一致。
...
避免 VS Code 因過度掃描大型資料夾而變慢。AI 工具（如 Copilot、Claude Code）會透過 `rg.exe`（ripgrep）搜尋專案，若未排除大型目錄，可能導致記憶體飆升甚至當機。
...
> **注意**: `search.exclude` 與 `files.watcherExclude` 是獨立的設定。前者防止搜尋掃描，後者防止檔案監控。兩者都需要設定才能完整防護。`.gitignore` **不會**影響 VS Code 的搜尋行為。

### 6. Rust 開發設定

若專案使用 Rust，建議排除 `target/` 編譯輸出目錄並啟用 rust-analyzer 相關功能。
...
```json
{
  ...
  "files.exclude": {
    ...
    "**/Thumbs.db": true,
    "**/desktop.ini": true,
    "**/$RECYCLE.BIN": true,   // (新增) 防止誤觸資源回收桶資料夾
    "**/System Volume Information": true, // (新增) 隱藏系統還原點資料夾
    "**/target": true          // (新增) Rust 編譯輸出目錄
  },

  // --- 檔案監控排除 (效能優化，避免 VS Code 因為掃描這些大資料夾而卡頓) ---
  "files.watcherExclude": {
    "**/.git/objects/**": true,
    "**/.git/subtree-cache/**": true,
    "**/node_modules/**": true,
    ...
    "**/.idea/**": true,
    "**/.vscode/**": true,
    "**/target/**": true,       // Rust 編譯輸出
    "**/models/**": true,       // ML 模型檔案
    "**/.venv/**": true,        // Python 虛擬環境
    "**/__pycache__/**": true,  // Python 快取
    "**/out/**": true,          // 通用編譯輸出
    "**/.next/**": true,        // Next.js
    "**/.nuxt/**": true,        // Nuxt.js
    "**/.output/**": true,      // Nuxt 3
    "**/coverage/**": true,     // 測試覆蓋率報告
    "**/.cache/**": true,       // 通用快取
  ...
  // --- 搜尋排除 (避免搜尋結果被編譯產物、AI 工具的 rg.exe 掃描干擾) ---
  "search.exclude": {
    "**/target": true,
    "**/node_modules": true,
    "**/logs": true,
  ...
  // --- 針對特定檔案類型的例外設定 (Override) ---

  // 1. Markdown: 關閉自動刪除行尾空白，因為 md 需要用兩個空白來換行
  "[markdown]": {
    "files.trimTrailingWhitespace": false
  },

  // 2. Windows Batch (.bat/.cmd): 強制使用 Big5 編碼 + CRLF，避免中文註解亂碼或執行失敗
  "[bat]": {
    "files.encoding": "cp950",
    "files.eol": "\r\n"
  },

  // 3. (選用) PowerShell: 雖然較新，但有時在舊版 Windows 預設仍非 UTF-8，可視需求加入
  "[powershell]": {
      "files.encoding": "utf8bom",
      "files.eol": "\r\n"
  },

  // --- Rust 開發設定 (選用，僅 Rust 專案需要) ---
  "rust-analyzer.cargo.buildScripts.enable": true,
  "rust-analyzer.procMacro.enable": true,
  "rust-analyzer.check.command": "check"
}
...
- **2026-02-20**: 大幅擴充效能優化排除清單（search.exclude / files.watcherExclude），涵蓋前端框架、Python、Rust、Java、Go、IaC 等常見大型目錄
- **2026-01-26**: 初版發布
