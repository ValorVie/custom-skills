---
description: (Optional) Derive ATDD acceptance test tables from approved SDD specification
...
## ⚠️ Optional Command | 可選命令

> **Important**: This command is **optional**. BDD scenarios (`.feature` files) already serve as executable acceptance tests and are generated by `/derive-all`.
>
> **重要**：此命令為**可選**。BDD 場景（`.feature` 檔案）已經是可執行的驗收測試，並由 `/derive-all` 生成。

### When to Use ATDD Tables | 何時使用 ATDD 表格

| Situation | Use /derive-atdd? |
|-----------|-------------------|
| Manual testing workflow required | ✅ Yes |
| Stakeholders prefer tabular format | ✅ Yes |
| Regulatory compliance requires specific test evidence | ✅ Yes |
| Standard development workflow | ❌ Use BDD instead |
| Automated testing primary | ❌ BDD is sufficient |

### Recommended Workflow | 建議的工作流程

```bash
# Preferred: Use /derive-all for BDD + TDD (sufficient for most cases)
/derive-all specs/SPEC-001.md

# Optional: Add ATDD tables only when specifically needed
