...
- **AND** 章節 SHALL 描述路徑：`mp-grill-with-docs`（必要時）→ `mp-to-issues`（Beads 出口）→ 直接實作（superpowers TDD）→ code review
...
- **AND** 三條軌道的執行狀態 SHALL 使用 Beads 追蹤
...
#### Scenario: canonical 執行出口一致

- **GIVEN** 輕量軌已納入工作流文件
- **WHEN** 檢查 `docs/agents/issue-tracker.md`
- **THEN** Beads SHALL 為輕量軌與正式變更的 canonical 執行追蹤器
- **AND** OpenSpec `tasks.md` SHALL 保留正式變更工件責任
- **AND** GitHub Issues SHALL 保留外部入口責任
