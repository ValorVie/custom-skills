# Spec: resource-disable

## Overview

實作資源停用機制，透過移動檔案來停用/啟用 skills、commands、agents。
...
- 依賴 `get_target_path()` 取得目標工具目錄路徑
- 依賴 `toggle-config.yaml` 記錄停用狀態
- 依賴現有的來源路徑函式（`get_uds_dir()`, `get_obsidian_skills_dir()` 等）
