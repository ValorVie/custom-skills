...
- 提案內容：建立一套以政策為主的通用 Agent Router。通用核心只判斷任務模式、
  ...
  專案規則由專案適配層加上。
- 目前狀態：外部方案比較已完成，本文件收斂後續討論形成的設計；尚未建立通用
  Skill，也未修改現有 `qdm-agent-team`。
- 本階段位置：先固定路由契約與分層，再到 `custom-skills` 做最小實作。
...
在 `custom-skills` 建立最小 `SKILL.md`，並讓既有工作流路由只增加一個觸發
指標。完成條件是上述案例可重複得到一致的路由形狀，且 Skill 內沒有 QDM 名稱
或任務追蹤器規則。
...
| 面向 | 影響 |
|------|------|
| 向下相容 | 新核心尚未接入，現有 QDM Skill 行為不變 |
| 程式碼 | 本 RFC 不新增執行程式碼 |
